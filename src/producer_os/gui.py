...
class EngineRunner(QObject):
    ...
    def _run(self) -> None:
        ...
        # Construct path to run report JSON
        run_id = report.get("run_id")
        hub_dir = self.engine.hub_dir
        report_path = hub_dir / "logs" / run_id / "run_report.json"
        self.finished.emit(report, str(report_path))
...
class ProducerOSWizard(QMainWindow):
    ...
    def _create_options_page(self) -> QWidget:
        ...
        info_label = QLabel(
            "FL Studio styling uses IconIndex only.  Sidecar .nfo files are
created automatically next to folders (not inside)."
        )
    ...
    def open_config_folder(self) -> None:
        ...
        QDesktopServices = getattr(__import__("PySide6.QtGui"), "QDesktopServices")  # lazy import
    ...
    def open_last_report(self) -> None:
        ...
        QDesktopServices = getattr(__import__("PySide6.QtGui"), "QDesktopServices")
